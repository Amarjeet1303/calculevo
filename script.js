const calculators = [
  ...
  },
  {
    ...
    calcFunc: function () {
      ...
      res.textContent = `Estimated BMR (calories/day): ${bmr.toFixed(0)}`;
    }
  },
  {
    id: "speed",
    ...
    html: `
      ...
      <input type="number" id="distance" min="0" step="any" placeholder="e.g., 100" />
      <label for="time">Time (hours):</label>
      <input type="number" id="time" min="0" step="any" placeholder="e.g., 2" />
    ...
    calcFunc: function () {
      ...
      if (!d || !t || d <= 0 || t <= 0) {
        res.textContent = "Please enter valid distance and time.";
        return;
      }
      const speed = d / t;
...
function loadCalculator(calcId) {
  ...
  if (btn) {
    ...
    // Also trigger calculation on Enter or Space key for accessibility
